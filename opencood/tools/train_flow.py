...
import subprocess
from opencood.models.flow_estimate import make_model
...
def train_parser():
    ...
    parser.add_argument('-False-fusion_method', '-f', default="intermediate",
...
def main():
    ...
    hypes = yaml_utils.load_yaml("opencood/hypes_yaml/v2xset/v2xset_flow_estimate.yaml", opt)
    ...
    # model, _ = make_model(input_dim=4, output_dim=2, num_layers=2) # num_layers指的是Encoder Decoder的个数
    model, _ = make_model(input_dim=3, output_dim=3, num_layers=2) # num_layers指的是Encoder Decoder的个数
    model = model.double()
    ...
    if 'is_single_pre_trained' in hypes and hypes['is_single_pre_trained']['pre_train_flag']: # 单车检测模型是否要加载预训练
        is_pre_trained = True
    if is_pre_trained:
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for i, batch_data in enumerate(train_loader): 
            ...
            #     hypes['model']['args']['with_compensation']: # 猜测有这个参数的意味着不需要运动补偿 也就是应该是ROI Generator 和 Final fuse model
