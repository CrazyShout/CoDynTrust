...
def main():
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for i, batch_data in enumerate(train_loader): 
            ...
            final_loss = criterion(ouput_dict, batch_data['ego']['single_object_label']) # 这个是单车做损失
            if i % 10 == 0:
                criterion.logging(epoch, i, len(train_loader), writer)
        ...
        if epoch % hypes['train_params']['eval_freq'] == 0:
            ...
            with torch.no_grad():
                for i, batch_data in tenumerate(val_loader):
                    ...
                    final_loss = criterion(ouput_dict, batch_data['ego']['single_object_label']) # 这个是单车做损失 2024年04月19日
