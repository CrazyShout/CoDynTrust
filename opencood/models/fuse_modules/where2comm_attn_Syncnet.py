...
class Where2comm_SyncNet(nn.Module):
    def __init__(self, args):
        super(Where2comm_SyncNet, self).__init__()
            ...
            for i in range(self.num_levels):
                ...
                if i==0:
                    if self.communication:
                        ...
                        # k>1的时候必须要手动选择past0的
                        H0, W0 =  communication_masks_tensor.shape[2:]
                        communication_masks_tensor = communication_masks_tensor.reshape(-1, K, 1, H0, W0) # (BxN,K, 1, H, W)
                ...
                batch_node_features = self.regroup(x, record_len, 1)# 2024年7月22日：原本这里是K，但是其实上面已经update，k帧化为了一帧
