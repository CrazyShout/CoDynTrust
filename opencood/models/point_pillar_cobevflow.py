...
class PointPillarWhere2commFlowTwoStageWDir(nn.Module):
    ...
    def generate_box_flow(self, data_dict, pred_dict, dataset, shape_list, device): 
        ...
        past_k_time_diff = self.regroup(data_dict['past_k_time_interval'], data_dict['record_len'], k=3) # k帧的时间间隔 长度为 (B) 返回List 每个元素为(N_b x k)  其实这里就是(N_b x 3)
        ...
        for b in range(B):
            ...
            '''
            for cav_idx in range(data_dict['record_len'][b]):# 遍历一个场景下的所有车
            ...
            if self.viz_bbx_flag: # 这个应该是判断是否需要可视化单车检测的bbx流数据
    ...
    def forward(self, data_dict, dataset=None):
        ...
        shape_list = torch.tensor(batch_dict['spatial_features'].shape[-3:]).to(pairwise_t_matrix.device) # [64, H, W]
        ...
        if self.design_mode != 4:
            ...
            if self.viz_bbx_flag: # 似乎是用来可视化特征的
        ...
        if self.multi_scale:
            if self.design_mode == 0 or self.design_mode==5:
                if self.viz_bbx_flag:
                    ...
                    fused_feature, communication_rates, result_dict = self.rain_fusion(batch_dict['spatial_features'], # (batch_cav_size, C, H, W)
                        psm_single, # (B, 2, H', W')
                        record_len, # (B, )
