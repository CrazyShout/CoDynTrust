...
class IntermediateFusionDatasetV2XSETFlowXYJNew(basedataset.BaseDataset):
    """
    This class is for intermediate fusion where each vehicle transmit the
    deep features to ego.
    """
    def __init__(self, params, visualize, train=True):
        print("start create a IntermediateFusionDatasetV2XSETFlowXYJNew object!")
        ...
        print(f"期望采样间隔为:{self.sample_interval_exp}")
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                for timestamp in timestamps:
                    ...
                    lidar_file = os.path.join(cav_path,
                                              timestamp + '.pcd')
                    camera_files = os.path.join(cav_path,
    ...
    def retrieve_base_data(self, idx):
        ...
        for cav_id, cav_content in scenario_database.items(): # 遍历一个场景下的所有cav
            ...
            # 用于可视化前视摄像头
            data[cav_id]['curr']['camera0'] = cav_content['regular'][timestamp_key]['camera0']
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, cav_content in base_data_dict.items():
            if cav_content['ego']:
                ...
                camera0_files = cav_content['curr']['camera0']
                break	
        ...
        for cav_id in cav_id_list: # 一个场景下遍历每一辆车
            ...
            if self.visualize:
                projected_lidar_stack.append(selected_cav_processed['projected_lidar']) # [(n1, 3), (n2, 3)...]
            if self.viz_bbx_flag:
                single_lidar_stack.append(selected_cav_processed['single_lidar']) # cur的每一个cav的点云信息(n, 3)
        ...
        if self.visualize:
            processed_data_dict['ego'].update({'origin_lidar':
                np.vstack(projected_lidar_stack)}) # (n1+n2+..., 3)
            split_num = []
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose, idx):
        ...
        if self.visualize:
            ...
            projected_lidar = \
                box_utils.project_points_by_matrix_torch(lidar_np[:, :3], transformation_matrix) # 点云投影过去 cur时间
        ...
        # print('past_k_time_diffs 在一辆cav上的结果: ', past_k_time_diffs)
        ...
        if self.is_generate_gt_flow:
            ...
            for t_i in range(2): # 遍历两次 过去一帧和当前帧 第一次为过去一帧， 第二次为当前帧
                ...
                # split_part = selected_cav_base['past_k'][0] # TODO: 这是为了测试无补偿的情况
                object_bbx_center, object_bbx_mask, object_ids = \
    ...
    def find_common_id(self, objct_ids):
        '''
    ...
    def collate_batch_test(self, batch):
        ...
        output_dict = self.collate_batch_train(batch) # 先和train一样的处理
    ...
    def post_process(self, data_dict, output_dict):
        ...
        pred_box_tensor, pred_score = \
            self.post_processor.post_process(data_dict, output_dict) # pred_box_tensor:(N, 8, 3) pred_score: (N,)
        gt_box_tensor = self.post_processor.generate_gt_bbx(data_dict) # （N, 8 , 3）

        return pred_box_tensor, pred_score, gt_box_tensor

    # 单车检测的时候推理会用到这里去NMS和生成预测bbx 2024年04月20日
    def post_process_no_fusion(self, data_dict, output_dict_ego, return_uncertainty=False):
        ...
        gt_box_tensor = self.post_processor.generate_gt_bbx(data_dict) # 生成gt bbx （n, 8 ,3）
